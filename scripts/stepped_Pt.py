...
n_epochs_per_loop = 1000        # number of epochs per training loop
temperatures = np.exp(np.linspace(np.log(5.0),np.log(0.02), 10))  # log spaced temperatures
samples_per_stage = 32         # number of evaluated samples per loop
buffer_size = 16
...
confinement_above_zmax = [0.0, 4.0]  # confinement above the maximum z position of the template 
...
##### TEMPLATE #####
def get_template():
    atoms = surface('Pt', (1,2,2), 5, vacuum=8.0)
...
def get_buffer(data, energies, T, N):
    ...
    e = [energy for energy in energies if energy < 0.0]
